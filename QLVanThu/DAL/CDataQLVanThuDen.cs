...
namespace QLVanThuDen.DAL
{
    class CDataQLVanThuDen
    {
        ...
        public DataTable LoadDSVanThuDen()
        {
            ...
            string sql = "select convert(varchar(10),d.ReceivedDate,103) as NgayDen,DocumentOrderNo as SoDen,IssuedOrganizationName2 as TacGiaVB,DocumentNo as SoKyHieuVB,convert(varchar(10),IssuedDate,103) as NgayThangVB,";
            ...
            sql += "from WF_Books b,WF_Incoming_Docs d,WF_Incoming_Doc_Files df,WF_Doc_Types t where b.BookID=d.BookID and d.TypeID=t.TypeID and d.DocumentID=df.DocumentID order by d.ReceivedDate desc,DocumentOrderNo desc";
            try 
            {
        ...
        public DataTable LoadDSVanThuDenDateToDate(string tungay,string denngay)
        {
            ...
            string sql = "select convert(varchar(10),d.ReceivedDate,103) as NgayDen,DocumentOrderNo as SoDen,IssuedOrganizationName2 as TacGiaVB,DocumentNo as SoKyHieuVB,convert(varchar(10),IssuedDate,103) as NgayThangVB,";
            sql += "t.Notation as LoaiVB,DocumentSummary as LoaiTrichYeuNoiDung,ReviewContent as NguoiNhan,d.DocumentID as ID,d.BookID as LoaiVBGID,b.Name as LoaiVBGName,df.FileName as PathFile ";
            sql += "from WF_Books b,WF_Incoming_Docs d,WF_Incoming_Doc_Files df,WF_Doc_Types t where d.ReceivedDate between '" + tungay + "' and '" + denngay + "' and b.BookID=d.BookID and d.TypeID=t.TypeID and d.DocumentID=df.DocumentID order by d.ReceivedDate desc,DocumentOrderNo desc";
            try
            {
                if (db.Connection.State == ConnectionState.Open)
                {
