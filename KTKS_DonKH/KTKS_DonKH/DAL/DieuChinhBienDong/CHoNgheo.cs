    ...
    class CHoNgheo : CDAL
    {
        ...
        public DataTable getBaoCao(int Nam, int FromKy, int ToKy)
        {
            string sql = "declare @Nam int=" + Nam + ",@FromKy int=" + FromKy + ",@ToKy int=" + ToKy
                    + " select Quan,Loai,SoHo=COUNT(distinct DanhBo),TieuThu=SUM(DinhMuc) from"
                    + " (select Quan=(select name from Quan where ID=Quan),Loai,t1.DanhBo,t2.DinhMuc from"
                    ...
                    + " (select DanhBa,Quan,DinhMuc=case when GB=10 then DM when GB!=10 and DinhMucHN is not null then DinhMucHN end"
                    + " from hoadon_ta.dbo.hoadon where NAM=@Nam and KY>=@FromKy and KY<=@ToKy)t2"
