    ...
    public partial class frmToTrinh2021 : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            try
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Xoa"))
                {
                    ...
                    {
                        string flagID = _cttt.IDCT.ToString();
                        var transactionOptions = new TransactionOptions();
                        transactionOptions.IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted;
                        using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, transactionOptions))
                        {
                            //DonTu_LichSu dtls = _cDonTu.get_LichSu("ThuMoi_ChiTiet", (int)_cttt.IDCT);
                            //if (dtls != null)
                            //{
        ...
        private void frmToTrinh_KeyUp(object sender, KeyEventArgs e)
        {
            if (_dontu_ChiTiet != null && e.Control && e.KeyCode == Keys.T)
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                {
                    frmCapNhatDonTu_Thumbnail frm = new frmCapNhatDonTu_Thumbnail(_dontu_ChiTiet, "ToTrinh_ChiTiet", _cttt.IDCT);
                    frm.ShowDialog();
                }
                else
                    MessageBox.Show("Bạn không có quyền Sửa Form này", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    if (dialog.FileName.ToLower().Contains("pdf"))
                        bytes = _cTT.scanFile(dialog.FileName);
                    else
                        bytes = _cTT.scanImage(dialog.FileName);
                    if (_cttt == null)
                    {
                        var index = dgvHinh.Rows.Add();
        ...
        private void dgvHinh_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                _cTT.LoadFileView("ToTrinh_ChiTiet_Hinh", _cttt.IDCT.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
            }
            else
            {
                byte[] hinh = _wsThuongVu.get_Hinh("ToTrinh_ChiTiet_Hinh", _cttt.IDCT.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
                if (hinh != null)
                    _cTT.LoadImageView(hinh);
                else
