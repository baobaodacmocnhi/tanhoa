        ...
        private void btnTVLapDon_Click(object sender, EventArgs e)
        {
            try
            {
                if (CTaiKhoan.CheckQuyen("mnuDCHD", "Them"))
                {
                    ...
                    {
                        if (_db.DieuChinhHangLoats.Any(item => item.Nam == int.Parse(dgvDanhSach.Rows[0].Cells["Nam"].ToString()) && item.Ky == int.Parse(dgvDanhSach.Rows[0].Cells["Ky"].ToString()) && item.Dot == int.Parse(dgvDanhSach.Rows[0].Cells["Dot"].ToString()) && item.MaDon != null) == true)
                        {
                        ...
                        {
                            HOADON hd = _cThuTien.GetMoiNhat(dgvDanhSach.Rows[i].Cells["DanhBo"].ToString());
        ...
        private void btnTVDieuChinh_Click(object sender, EventArgs e)
        {
            try
            {
                if (CTaiKhoan.CheckQuyen("mnuDCHD", "Them"))
                {
                    if (MessageBox.Show("Bạn có chắc chắn?", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                    {
                        if (_db.DieuChinhHangLoats.Count(item => item.Nam == int.Parse(dgvDanhSach.Rows[0].Cells["Nam"].ToString()) && item.Ky == int.Parse(dgvDanhSach.Rows[0].Cells["Ky"].ToString()) && item.Dot == int.Parse(dgvDanhSach.Rows[0].Cells["Dot"].ToString()) && item.MaDon == null) == dgvDanhSach.Rows.Count)
                        {
                            MessageBox.Show("Chưa có Mã Đơn", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                        if (_db.DieuChinhHangLoats.Any(item => item.Nam == int.Parse(dgvDanhSach.Rows[0].Cells["Nam"].ToString()) && item.Ky == int.Parse(dgvDanhSach.Rows[0].Cells["Ky"].ToString()) && item.Dot == int.Parse(dgvDanhSach.Rows[0].Cells["Dot"].ToString()) && item.DCHD == true) == true)
                        {
                        ...
                        for (int i = 0; i < dgvDanhSach.Rows.Count; i++)
                        {
                            if (dgvDanhSach.Rows[i].Cells["MaDon"].ToString() != "")
                            {
                                DonTu_ChiTiet dontu_ChiTiet = _cDonTu.get_ChiTiet(int.Parse(dgvDanhSach.Rows[i].Cells["MaDon"].ToString()), int.Parse(dgvDanhSach.Rows[i].Cells["STT"].ToString()));
                                ...
                                if (_cDCBD.checkExist_HoaDon(dontu_ChiTiet.MaDon.Value, dontu_ChiTiet.DanhBo, int.Parse(dgvDanhSach.Rows[i].Cells["Ky"].ToString()).ToString("00") + "/" + int.Parse(dgvDanhSach.Rows[i].Cells["Nam"].ToString())) == false)
                                {
                                    ...
                                    ctdchd.KyHD = int.Parse(dgvDanhSach.Rows[i].Cells["Ky"].ToString()).ToString("00") + "/" + int.Parse(dgvDanhSach.Rows[i].Cells["Nam"].ToString());
                                    HOADON hd = _cThuTien.Get(dontu_ChiTiet.DanhBo, int.Parse(dgvDanhSach.Rows[i].Cells["Ky"].ToString()), int.Parse(dgvDanhSach.Rows[i].Cells["Nam"].ToString()));
                                    DocSo ds = _cDocSo.get(dontu_ChiTiet.DanhBo, int.Parse(dgvDanhSach.Rows[i].Cells["Ky"].ToString()), int.Parse(dgvDanhSach.Rows[i].Cells["Nam"].ToString()));
                                    if (hd != null)
                                        ctdchd.Dot = hd.DOT;
                                    else
                                        if (ds != null)
