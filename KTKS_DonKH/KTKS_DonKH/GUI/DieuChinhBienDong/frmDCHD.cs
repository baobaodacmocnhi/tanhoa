...
namespace KTKS_DonKH.GUI.DieuChinhBienDong
{
    public partial class frmDCHD : Form
    {
                    ...
                    using (TransactionScope scope = new TransactionScope())
                        if (_cDCBD.ThemDCHD(ctdchd))
                        {
                            ...
                            foreach (DataGridViewRow item in dgvHinh.Rows)
                            {
                                ...
                                if (_wsThuongVu.ghi_Hinh("DCBD_ChiTietHoaDon_Hinh", en.IDDCBD_ChiTietHoaDon.Value.ToString(), en.Name + ".jpg", Convert.FromBase64String(item.Cells["Bytes_Hinh"].Value.ToString())) == true)
                                _cDCBD.Them_Hinh(en);
                            }
                            if (_dontu_ChiTiet != null)
                            {
        ...
        private void TinhTienNuoc()
        {
            ...
            int Ky = 0, Nam = 0, TyleSH = 0, TyLeSX = 0, TyLeDV = 0, TyLeHCSN = 0, TongTienG1Truoc = 0, TongTienG2Truoc = 0, TienNuocTruoc = 0, ThueGTGTTruoc = 0, TDVTNTruoc = 0, ThueTDVTNTruoc = 0, TongTienG1Sau = 0, TongTienG2Sau = 0, TienNuocSau = 0, ThueGTGTSau = 0, TDVTNSau = 0, ThueTDVTNSau = 0, TieuThu_DieuChinhGia = 0, PhiBVMTG1Truoc = 0, PhiBVMTG2Truoc = 0, PhiBVMTG1Sau = 0, PhiBVMTG2Sau = 0;
                ...
                try
                {
                    ...
                    else
                    {
                        if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                        {
                            ...
                            if (_wsThuongVu.ghi_Hinh("DCBD_ChiTietHoaDon_Hinh", en.IDDCBD_ChiTietHoaDon.Value.ToString(), en.Name + ".jpg", bytes) == true)
        ...
        private void dgvHinh_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (dgvHinh.CurrentRow.Cells["Bytes_Hinh"].Value != null && dgvHinh.CurrentRow.Cells["Bytes_Hinh"].Value.ToString() != "")
                ...
                _cDCBD.LoadImageView(_wsThuongVu.get_Hinh("DCBD_ChiTietHoaDon_Hinh", _ctdchd.MaCTDCHD.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + ".jpg"));
        }
                    ...
                    if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                    {
                        if (MessageBox.Show("Bạn có chắc chắn xóa?", "Xác nhận xóa", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            if (dgvHinh.CurrentRow.Cells["ID_Hinh"].Value != null)
                                if (_wsThuongVu.xoa_Hinh("DCBD_ChiTietHoaDon_Hinh", _ctdchd.MaCTDCHD.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + ".jpg") == true)
