...
namespace KTKS_DonKH.GUI.DieuChinhBienDong
{
    public partial class frmBaoCaoDCBD : Form
    {
        ...
        private void btnBaoCao_Click(object sender, EventArgs e)
        {
            if (radDSDMCapCoThoiHan.Checked)
            {
                ...
                foreach (DataRow itemRow in dt.Rows)
                {
                    ...
                    if (cmbQuan.SelectedValue.ToString() == "0")
                    {
                        ...
                        dr["Phuong"] = _cDCBD.getTenPhuong(int.Parse(hoadon.Quan), int.Parse(hoadon.Phuong));
                        dr["Quan"] = _cDCBD.getTenQuan(int.Parse(hoadon.Quan));

                        dsBaoCao.Tables["DSCapDinhMuc"].Rows.Add(dr);
                    }
                    else
                        if (int.Parse(cmbQuan.SelectedValue.ToString()) == int.Parse(hoadon.Quan))
                            if (int.Parse(cmbPhuong.SelectedValue.ToString()) == 0)
                            {
                                DataRow dr = dsBaoCao.Tables["DSCapDinhMuc"].NewRow();
                                ...
                                if (int.Parse(cmbPhuong.SelectedValue.ToString()) == int.Parse(hoadon.Phuong))
                                {
                                    DataRow dr = dsBaoCao.Tables["DSCapDinhMuc"].NewRow();
            ...
            if (radThongKeDCSoCT.Checked)
            {
                ...
                foreach (DataRow itemRow in dt.Rows)
                {
                    ...
                    dr["Phuong"] = _cDCBD.getTenPhuong(int.Parse(itemRow["Quan"].ToString()), int.Parse(itemRow["Phuong"].ToString()));
                    dr["Quan"] = _cDCBD.getTenQuan(int.Parse(itemRow["Quan"].ToString()));
        ...
        private void btnBaoCao_ThongKeDC_Click(object sender, EventArgs e)
        {
            ...
            if (chkChiLayTrongKy_ThongKeDC.Checked == false)
            {
            ...
            else
            {
                if (int.Parse(cmbQuan_ThongKeDC.SelectedValue.ToString()) == 0)
                {
                    dtDCBD = _cDCBD.getDS_BienDong_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim());
                    dtDCHD = _cDCBD.getDS_HoaDon_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim());
                }
                else
                    if (int.Parse(cmbPhuong_ThongKeDC.SelectedValue.ToString()) == 0)
                    {
                        dtDCBD = _cDCBD.getDS_BienDong_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim(), int.Parse(cmbQuan_ThongKeDC.SelectedValue.ToString()));
                        dtDCHD = _cDCBD.getDS_HoaDon_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim(), int.Parse(cmbQuan_ThongKeDC.SelectedValue.ToString()));
                    }
                    else
                    {
                        dtDCBD = _cDCBD.getDS_BienDong_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim(), int.Parse(cmbQuan_ThongKeDC.SelectedValue.ToString()), int.Parse(cmbPhuong_ThongKeDC.SelectedValue.ToString()));
                        dtDCHD = _cDCBD.getDS_HoaDon_HieuLucKy(txtHieuLucKy_ThongKeDC.Text.Trim(), int.Parse(cmbQuan_ThongKeDC.SelectedValue.ToString()), int.Parse(cmbPhuong_ThongKeDC.SelectedValue.ToString()));
                    }
            }
                ...
                else
                {
                    ...
                    foreach (DataRow itemRow in dtDCBD.Rows)
                        if (itemRow["HieuLucKy"].ToString() != "")
                        {
                            ...
                            {
                                MessageBox.Show("Số Phiếu: " + itemRow["ID"].ToString().Insert(itemRow["ID"].ToString().Length - 2, "-") + " sai hiệu lực kỳ", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                }
            ...
            else
            {
                foreach (DataRow itemRow in dtDCBD.Rows)
                {
                    ...
                    dr["TuNgay"] = "Trong kỳ " + txtHieuLucKy_ThongKeDC.Text.Trim();
                    //dr["DenNgay"] = dateDen_ThongKeDC.Value.Date.ToString("dd/MM/yyyy");
                    if (string.IsNullOrEmpty(itemRow["SoCongVan"].ToString()) == true)
                        dr["SoCongVan"] = "(theo đơn khách hàng)";
                    else
        ...
        private void btnHoNgheo_Click(object sender, EventArgs e)
        {
            DataTable dt = _cHoNgheo.getBaoCao(int.Parse(cmbNam.SelectedValue.ToString()), int.Parse(cmbFromKy.SelectedItem.ToString()), int.Parse(cmbToKy.SelectedItem.ToString()));
            //Tạo các đối tượng Excel
            Microsoft.Office.Interop.Excel.Application oExcel = new Microsoft.Office.Interop.Excel.Application();
            Microsoft.Office.Interop.Excel.Workbooks oBooks;
            Microsoft.Office.Interop.Excel.Sheets oSheets;
            ...
            oSheet.Name = "Hộ Nghèo";

            // Tạo phần đầu nếu muốn
            Microsoft.Office.Interop.Excel.Range head = oSheet.get_Range("A1", "F1");
            head.MergeCells = true;
            head.Value2 = "Hộ Nghèo Kỳ " + cmbFromKy.SelectedItem.ToString() + " - " + cmbToKy.SelectedItem.ToString() + "/" + cmbNam.SelectedValue.ToString();
            head.Font.Bold = true;
            head.Font.Name = "Times New Roman";
            head.Font.Size = "20";
            head.RowHeight = 50;
