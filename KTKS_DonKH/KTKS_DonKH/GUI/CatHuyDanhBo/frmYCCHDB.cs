...
namespace KTKS_DonKH.GUI.CatHuyDanhBo
{
    public partial class frmYCCHDB : Form
    {
        ...
        public void LoadPhieuCHDB(CHDB_Phieu phieuCHDB)
        {
            ...
            txtNoiNhan.Text = phieuCHDB.NoiNhan;
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Them"))
                {
                    ...
                    ycchdb.NoiNhan = txtNoiNhan.Text.Trim();
                    ...
                    using (TransactionScope scope = new TransactionScope())
                        if (_cCHDB.ThemPhieuHuy(ycchdb))
                        {
                            foreach (DataGridViewRow item in dgvHinh.Rows)
                            {
                                ...
                                if (_wsThuongVu.ghi_Hinh("CHDB_Phieu_Hinh", en.IDCHDB_Phieu.Value.ToString(), en.Name + en.Loai, Convert.FromBase64String(item.Cells["Bytes_Hinh"].Value.ToString())) == true)
                                    _cCHDB.Them_Hinh(en);
                            }
                            if (_dontu_ChiTiet != null)
                            {
            ...
            try
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                {
                    if (_ycchdb != null)
                    {
                        ...
                        _ycchdb.NoiNhan = txtNoiNhan.Text.Trim();
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (_ycchdb != null && MessageBox.Show("Bạn có chắc chắn???", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                    {
                        ...
                        {
                            DonTu_LichSu dtls = _cDonTu.get_LichSu("CHDB_Phieu", (int)_ycchdb.MaYCCHDB, _ycchdb.CreateBy.Value);
                            if (dtls != null)
                            {
                ...
                try
                {
                    ...
                    else
                    {
                        if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                        {
                            ...
                            if (_wsThuongVu.ghi_Hinh("CHDB_Phieu_Hinh", en.IDCHDB_Phieu.Value.ToString(), en.Name + en.Loai, bytes) == true)
        ...
        {
            byte[] file = _wsThuongVu.get_Hinh("CHDB_Phieu_Hinh", _ycchdb.MaYCCHDB.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
            if (file != null)
                    ...
                    if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                    {
                        if (MessageBox.Show("Bạn có chắc chắn???", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            if (dgvHinh.CurrentRow.Cells["ID_Hinh"].Value != null)
                                if (_wsThuongVu.xoa_Hinh("CHDB_Phieu_Hinh", _ycchdb.MaYCCHDB.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString()) == true)
