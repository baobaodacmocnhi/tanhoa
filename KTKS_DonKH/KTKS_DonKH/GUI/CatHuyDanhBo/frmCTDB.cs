...
namespace KTKS_DonKH.GUI.CatHuyDanhBo
{
    public partial class frmCTDB : Form
    {
        ...
        public void LoadCTDB(CHDB_ChiTietCatTam ctctdb)
        {
            ...
            if (ctctdb.NgayThucHien != null)
                dateThucHien.Value = ctctdb.NgayThucHien.Value;
            ...
            if (CTaiKhoan.CheckQuyen(_mnu, "Them"))
            {
                try
                {
                    ...
                    if (txtSoTien.Text.Trim() != "")
                        ctctdb.SoTien = long.Parse(txtSoTien.Text.Trim().Replace(".", ""));
                    ...
                    ctctdb.NgayThucHien = dateThucHien.Value;
                    ///Ký Tên
                    ...
                    using (TransactionScope scope = new TransactionScope())
                        if (_cCHDB.ThemCTCTDB(ctctdb))
                        {
                            foreach (DataGridViewRow item in dgvHinh.Rows)
                            {
                                ...
                                if (_wsThuongVu.ghi_Hinh("CHDB_ChiTietCatTam_Hinh", en.IDCHDB_ChiTietCatTam.Value.ToString(), en.Name + en.Loai, Convert.FromBase64String(item.Cells["Bytes_Hinh"].Value.ToString())) == true)
                                    _cCHDB.Them_Hinh(en);
                            }
                            if (_dontu_ChiTiet != null)
                            {
            ...
            if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
            {
                try
                {
                    if (_ctctdb != null)
                    {
                        ...
                        if (txtSoTien.Text.Trim() != "")
                            _ctctdb.SoTien = long.Parse(txtSoTien.Text.Trim().Replace(".", ""));
                        else
                            _ctctdb.SoTien = null;
                        if (cmbViTriDHN1.SelectedItem != null)
                        ...
                        _ctctdb.NgayThucHien = dateThucHien.Value;
                        ///
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                if (_ctctdb != null && MessageBox.Show("Bạn có chắc chắn???", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    string flagID = _ctctdb.MaCTCTDB.ToString();
                    ...
                    using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, transactionOptions))
                    {
                        ...
                        {
                            _wsThuongVu.xoa_Folder_Hinh("CHDB_ChiTietCatTam_Hinh", flagID);
                            scope.Complete();
                            scope.Dispose();
                            MessageBox.Show("Thành công", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Clear();
        ...
        private void frmCTDB_KeyUp(object sender, KeyEventArgs e)
        {
            if (_dontu_ChiTiet != null && e.Control && e.KeyCode == Keys.T)
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                {
                    frmCapNhatDonTu_Thumbnail frm = new frmCapNhatDonTu_Thumbnail(_dontu_ChiTiet, "CHDB_ChiTietCatTam", (int)_ctctdb.MaCTCTDB);
                    frm.ShowDialog();
                }
                else
                    MessageBox.Show("Bạn không có quyền Sửa Form này", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    ...
                    else
                    {
                        if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                        {
                            ...
                            if (_wsThuongVu.ghi_Hinh("CHDB_ChiTietCatTam_Hinh", en.IDCHDB_ChiTietCatTam.Value.ToString(), en.Name + en.Loai, bytes) == true)
        ...
        private void dgvHinh_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                _cCHDB.LoadFileView("CHDB_ChiTietCatTam_Hinh", _ctctdb.MaCTCTDB.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
            }
            else
            {
                byte[] hinh = _wsThuongVu.get_Hinh("CHDB_ChiTietCatTam_Hinh", _ctctdb.MaCTCTDB.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
                if (hinh != null)
                    ...
                    if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                    {
                        if (MessageBox.Show("Bạn có chắc chắn???", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            if (dgvHinh.CurrentRow.Cells["ID_Hinh"].Value != null)
                                if (_wsThuongVu.xoa_Hinh("CHDB_ChiTietCatTam_Hinh", _ctctdb.MaCTCTDB.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString()) == true)
        ...
        private void btnInThongBaoMoi_Click(object sender, EventArgs e)
        {
            if (_ctctdb != null)
            {
                ...
                if (_ctctdb.GhiChuLyDo != "")
                    dr["LyDo"] += _ctctdb.GhiChuLyDo + ". ";
                if (_ctctdb.SoTien.ToString() != "")
                    dr["LyDo"] += "\r\nTổng số tiền: " + String.Format(CultureInfo.CreateSpecificCulture("vi-VN"), "{0:#,## đồng}", _ctctdb.SoTien);
                dr["NoiDung"] = _ctctdb.NoiDung;
