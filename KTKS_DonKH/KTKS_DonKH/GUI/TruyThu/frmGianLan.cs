...
namespace KTKS_DonKH.GUI.TruyThu
{
    public partial class frmGianLan : Form
    {
        ...
        public void LoadGianLan(GianLan_ChiTiet entity)
        {
            ...
            cmbTinhTrang.SelectedItem = entity.TinhTrang;
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if (CTaiKhoan.CheckQuyen(_mnu, "Them"))
            {
                try
                {
                    ...
                    {
                        entity.TinhTrang = cmbTinhTrang.SelectedItem.ToString();
                    }
                    else
                        entity.TinhTrang = "";
                    ...
                    using (TransactionScope scope = new TransactionScope())
                        if (_cGianLan.Them_ChiTiet(entity))
                        {
                            foreach (DataGridViewRow item in dgvHinh.Rows)
                            {
                                ...
                                if (_wsThuongVu.ghi_Hinh("GianLan_ChiTiet_Hinh", en.IDGianLan_ChiTiet.Value.ToString(), en.Name + en.Loai, Convert.FromBase64String(item.Cells["Bytes_Hinh"].Value.ToString())) == true)
            ...
            if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
            {
                try
                {
                    if (_gianlan != null)
                    {
                        ...
                        if (cmbTinhTrang.SelectedIndex != -1)
                        {
                            _gianlan.TinhTrang = cmbTinhTrang.SelectedItem.ToString();
                        }
                        else
                            _gianlan.TinhTrang = "";
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            switch (cmbTimTheo.SelectedItem.ToString())
            {
                ...
                case "Ngày":
                    ...
                    break;
                case "Tình Trạng":
                    if (cmbTinhTrang_TimKiem.SelectedIndex == -1)
                        return;
                    DataTable dt = new DataTable();
                    if (cmbTinhTrang_TimKiem.SelectedIndex == 0)
                        dt = _cGianLan.getDS_ChiTiet();
                    else
                        if (cmbTinhTrang_TimKiem.SelectedIndex > 0)
                            dt = _cGianLan.getDS_ChiTiet_TinhTrang(cmbTinhTrang_TimKiem.SelectedItem.ToString());
                    dgvGianLan.DataSource = dt;
                    break;
                default:
                    break;
            }
        ...
        private void cmbTimTheo_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (cmbTimTheo.SelectedItem.ToString())
            {
                case "Danh Bộ":
                    ...
                    cmbTinhTrang_TimKiem.Visible = false;
                    break;
                case "Ngày":
                ...
                case "Tình Trạng":
                    txtNoiDungTimKiem.Visible = false;
                    panel_KhoangThoiGian.Visible = false;
                    cmbTinhTrang_TimKiem.Visible = true;
                    break;
                default:
                    txtNoiDungTimKiem.Visible = false;
                    panel_KhoangThoiGian.Visible = false;
        ...
        private void frmGianLan_KeyUp(object sender, KeyEventArgs e)
        {
            if (_dontu_ChiTiet != null && e.Control && e.KeyCode == Keys.T)
            {
                if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                {
                    frmCapNhatDonTu_Thumbnail frm = new frmCapNhatDonTu_Thumbnail(_dontu_ChiTiet, "GianLan_ChiTiet", _gianlan.MaCTGL);
                    frm.ShowDialog();
                }
                else
                    MessageBox.Show("Bạn không có quyền Sửa Form này", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    if (dialog.FileName.ToLower().Contains("pdf"))
                        bytes = _cGianLan.scanFile(dialog.FileName);
                    else
                        bytes = _cGianLan.scanImage(dialog.FileName);
                    if (_gianlan == null)
                    {
                        var index = dgvHinh.Rows.Add();
                    ...
                    else
                    {
                        if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                        {
                            ...
                            if (_wsThuongVu.ghi_Hinh("GianLan_ChiTiet_Hinh", en.IDGianLan_ChiTiet.Value.ToString(), en.Name + en.Loai, bytes) == true)
        ...
        private void dgvHinh_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                _cGianLan.LoadFileView("GianLan_ChiTiet_Hinh", _gianlan.MaCTGL.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
            }
            else
            {
                byte[] hinh = _wsThuongVu.get_Hinh("GianLan_ChiTiet_Hinh", _gianlan.MaCTGL.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
                if (hinh != null)
                    _cGianLan.LoadImageView(hinh);
                else
                    ...
                    if (CTaiKhoan.CheckQuyen(_mnu, "Sua"))
                    {
                        if (MessageBox.Show("Bạn có chắc chắn???", "Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        {
                            if (dgvHinh.CurrentRow.Cells["ID_Hinh"].Value != null)
                                if (_wsThuongVu.xoa_Hinh("GianLan_ChiTiet_Hinh", _gianlan.MaCTGL.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString()) == true)
