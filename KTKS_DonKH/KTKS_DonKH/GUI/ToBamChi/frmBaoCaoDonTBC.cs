...
namespace KTKS_DonKH.GUI.ToBamChi
{
    public partial class frmBaoCaoDonTBC : Form
    {
        ...
        {
            cmbLD_DSLoaiDon.DataSource = _cLoaiDonTBC.GetDS();
        ...
        private void btnBaoCao_DSChuyenVP_Click(object sender, EventArgs e)
        {
            ...
            {
                case "Ngày":
                    dt = _cLichSuDonTu.GetDSChuyen_VP("TBC", dateTu_DSChuyenVP.Value, dateDen_DSChuyenVP.Value);
                    break;
                case "Số Công Văn":
                    dt = _cLichSuDonTu.GetDSChuyen_VP("TBC", txtNoiDungTimKiem_DSChuyenVP.Text.Trim().ToUpper());
                    break;
            }
        ...
        private void btnBaoCao_DSLoaiDon_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = _cDonTBC.GetDS(int.Parse(cmbLD_DSLoaiDon.SelectedValue.ToString()),dateTu_DSLoaiDon.Value,dateDen_DSLoaiDon.Value);
                DataSetBaoCao dsBaoCao = new DataSetBaoCao();
                foreach (DataRow item in dt.Rows)
                {
                    DataRow dr = dsBaoCao.Tables["DSDonTXL"].NewRow();
