...
select t1.DanhBo,DinhMuc_BD from
(select DanhBo,DinhMuc_BD,ROW_NUMBER() OVER (PARTITION BY DanhBo ORDER BY CreateDate DESC) AS rn from SERVER9.KTKS_DonKH.DBO.CTDCBD
...
and MaCTDCBD>=@FromMa and MaCTDCBD<=@ToMa and ChuyenDocSo=1 and DinhMuc_BD is not null) t1,
(select * from #tempDocSo) t2
where t1.DanhBo=t2.DanhBo and rn=1 and t1.DinhMuc_BD!=t2.DinhMuc
...
begin
	update SERVER8.CAPNUOCTANHOA.DBO.TB_DULIEUKHACHHANG set DinhMuc=@Value where DanhBo=@DanhBo
	fetch next
	from @getCursor into @DanhBo,@Value
end
close @getCursor
