    ...
    public partial class frmNhom : Form
    {
            ...
            if (CUser.CheckQuyen(_mnu, "Them"))
            {
                if (txtTenNhom.Text.Trim() != "")
                {
                    ...
                    foreach (var item in _cMenu.GetDS())
                    {
                        ...
                        phanquyennhom.IDMenu = item.ID;
                        phanquyennhom.IDNhom = nhom.ID;
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            if (CUser.CheckQuyen(_mnu, "Sua"))
            {
                if (_selectedindex != -1)
                {
                    Nhom nhom = _cNhom.get(int.Parse(dgvNhom["ID", _selectedindex].Value.ToString()));
                    nhom.Name = txtTenNhom.Text.Trim();
                    _cNhom.Sua(nhom);
                    DataTable dt = ((DataView)gridView.DataSource).Table;
                    foreach (DataRow item in dt.Rows)
                    {
                        PhanQuyenNhom phanquyennhom = _cPhanQuyenNhom.get(int.Parse(item["IDMenu"].ToString()), nhom.ID);
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {      
                    if (MessageBox.Show("Bạn có chắc chắn xóa?", "Xác nhận xóa", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                        if (_selectedindex != -1)
                        {
                            Nhom nhom = _cNhom.get(int.Parse(dgvNhom["MaNhom", _selectedindex].Value.ToString()));
                            ///xóa quan hệ 1 nhiều
        ...
        private void dgvNhom_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                txtTenNhom.Text = dgvNhom["Namee", e.RowIndex].Value.ToString();
                if(CUser.Admin)
                gridControl.DataSource = _cPhanQuyenNhom.getDS(true,int.Parse(dgvNhom["ID", e.RowIndex].Value.ToString()));
                else
                    gridControl.DataSource = _cPhanQuyenNhom.getDS(false, int.Parse(dgvNhom["ID", e.RowIndex].Value.ToString()));
            }
            catch (Exception)
            {
            }
        ...
        {
            //dgvDangNhap.DataSource = _cUser.getDS_DangNhap();
        }
