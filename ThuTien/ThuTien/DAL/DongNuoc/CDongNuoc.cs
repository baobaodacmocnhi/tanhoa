    ...
    class CDongNuoc : CDAL
    {
        ...
        public DataTable GetBaoCaoTongHop_DongNuoc_DanhSach(int MaTo, DateTime FromDate, DateTime ToDate)
        {
            string sql = "declare @FromDate date;"
                        ...
                        + " (select MaND,HoTen,STT from TT_NguoiDung where DongNuoc=1 and MaTo=1) nd,"
                        ...
                        + " and CAST(phoihop.CreateDate as date)>=@FromDate and CAST(phoihop.CreateDate as date)<=@ToDate) phoihop"
