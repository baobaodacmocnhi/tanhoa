...
namespace ThuTien.DAL.DongNuoc
{
    class CDongNuoc : CDAL
    {
        ...
        {
            DataTable dt = new DataTable();
            if (_db.TT_Tos.SingleOrDefault(itemA => itemA.MaTo == MaTo).DongNuoc == false)
            {
        ...
        public DataTable GetBaoCaoTongHop(DateTime FromDate, DateTime ToDate)
        {
        ...
        public DataTable GetBaoCaoTongHop(DateTime FromDate, DateTime ToDate, int FromDot, int ToDot)
        {
            string sql = "declare @FromDate date;"
                        + " declare @ToDate date;"
                        + " declare @TuCuonGCS int;"
                        ...
                        + " set @FromDate='" + FromDate.ToString("yyyyMMdd") + "';"
                        + " set @ToDate='" + ToDate.ToString("yyyyMMdd") + "';"
                        + " set @TuCuonGCS=" + FromDot
                        + " set @DenCuonGCS=" + ToDot
                        + " select nd.MaND as MaNV,nd.HoTen,nd.STT,toncu.DCTonCu,toncu.HDTonCu,toncu.TCTonCu,nhan.DCNhan,nhan.HDNhan,nhan.TCNhan"
                        + ",dangngan.DCDangNgan,dangngan.HDDangNgan,dangngan.TCDangNgan,lenhhuy.DCHuy,lenhhuy.HDHuy,lenhhuy.TCHuy,tongton.DCTongTon,tongton.HDTongTon,tongton.TCTongTon,dongnuoc.DCDongNuoc from"
                        + " (select MaND,HoTen,STT from TT_NguoiDung where DongNuoc=1) nd"
                        + " left join"
        ...
        public DataTable GetBaoCaoTongHop_ChiTiet(DateTime FromDate, DateTime ToDate)
        {
            string sql = "declare @FromDate date;"
                        + " declare @ToDate date;"
                        + " set @FromDate='" + FromDate.ToString("yyyyMMdd") + "';"
        ...
        public DataTable GetBaoCaoTongHop_ChiTiet(DateTime FromDate, DateTime ToDate, int FromDot, int ToDot)
        {
            string sql = "declare @FromDate date;"
                        + " declare @ToDate date;"
                        + " declare @TuCuonGCS int;"
        ...
        public DataTable GetBaoCaoTongHop_DanhSach(DateTime FromDate, DateTime ToDate)
        {
            string sql = "declare @FromDate date;"
                        + " declare @ToDate date;"
                        + " set @FromDate='" + FromDate.ToString("yyyyMMdd") + "';"
                        ...
                        + " where CAST(phoihop.CreateDate as date)>=@FromDate and CAST(phoihop.CreateDate as date)<=@ToDate and phoihop.CreateBy=nd.MaND) phoihop"
        ...
        public DataTable GetBaoCaoTongHop_DanhSach(int MaTo, DateTime FromDate, DateTime ToDate)
        {
            string sql = "declare @FromDate date;"
                        ...
                        + " (select distinct Loai=N'Đăng Ngân',nd.MaND,dn.DanhBo,dn.HoTen,dn.DiaChi,NgayXuLy=CAST(hd.NGAYGIAITRACH as date),NiemChi=NULL,DongNuoc2='false',NgayXuLy1=NULL,NiemChi1=NULL,KhoaTu='false',KhoaKhac='false',NoiDung=NULL"
                        + " from TT_DongNuoc dn,TT_CTDongNuoc ctdn,HOADON hd,TT_NguoiDung nd"
                        + " where dn.MaDN=ctdn.MaDN and ctdn.MaHD=hd.ID_HOADON and dn.MaNV_DongNuoc=nd.MaND and dn.Huy=0 and hd.MAY>=@TuCuonGCS and hd.MAY<=@DenCuonGCS"
                        + " and CAST(hd.NGAYGIAITRACH as date)>=@FromDate and CAST(hd.NGAYGIAITRACH as date)<=@ToDate) dangngan"
                        + " where nd.MaND=dangngan.MaND"
                        + " union all"
                        + " select nd.MaND as MaNV,nd.HoTen,nd.STT,dongnuoc.* from"
        ...
        public DataTable GetBaoCaoTongHop_DanhSach(DateTime FromDate, DateTime ToDate, int FromDot, int ToDot)
        {
            string sql = "declare @FromDate date;"
                        + " declare @ToDate date;"
                        + " declare @TuCuonGCS int;"
        ...
        public DataTable getDSKQDongNuoc_ChuaDongPhi()
        {
            ...
            //            };
            //return LINQToDataTable(query.GroupBy(item => item.MaDN).Select(item => item.First()).ToList());
            return ExecuteQuery_DataTable("select distinct MaKQDN,kqdn.MaDN,kqdn.CreateDate,kqdn.DanhBo,kqdn.HoTen,kqdn.DiaChi,NgayDN"
                        + " ,NGAYGIAITRACH=(select NGAYGIAITRACH from HOADON where ID_HOADON=(select top 1 MaHD from TT_CTDongNuoc where MaDN=dn.MaDN)),"
                        + " CoDHN = Co,kqdn.GhiChuTroNgai,TroNgaiMN,MaNV_DongNuoc=(select HoTen from TT_NguoiDung where MaND=dn.MaNV_DongNuoc)"
                        + " from TT_DongNuoc dn,TT_KQDongNuoc kqdn,TT_CTDongNuoc ctdn"
                        + " where dn.MaDN=kqdn.MaDN and dn.MaDN=ctdn.MaDN and NgayDN is not null and DongPhi=0 and NgayMN is null and TroNgaiMN=0"
                        + " and (select count(*) from TT_CTDongNuoc a,HOADON hd where a.MaDN=dn.MaDN and a.MaHD=hd.ID_HOADON and ChuyenNoKhoDoi=0 and NGAYGIAITRACH is not null)=(select count(*) from TT_CTDongNuoc where MaDN=dn.MaDN)");
        }
        ...
        public DataTable getDSKQDongNuoc_ChuaDongPhi(DateTime FromNgayDN, DateTime ToNgayDN)
        {
            ...
            return ExecuteQuery_DataTable("select distinct MaKQDN,kqdn.MaDN,kqdn.CreateDate,kqdn.DanhBo,kqdn.HoTen,kqdn.DiaChi,NgayDN"
                        ...
                        + " from TT_DongNuoc dn,TT_KQDongNuoc kqdn,TT_CTDongNuoc ctdn"
                        + " where dn.MaDN=kqdn.MaDN and dn.MaDN=ctdn.MaDN and NgayDN is not null and DongPhi=0 and NgayMN is null and TroNgaiMN=0"
                        + " and cast(kqdn.NgayDN as date)>='" + FromNgayDN.ToString("yyyyMMdd") + "' and cast(kqdn.NgayDN as date)<='" + ToNgayDN.ToString("yyyyMMdd") + "'"
                        + " and (select count(*) from TT_CTDongNuoc a,HOADON hd where a.MaDN=dn.MaDN and a.MaHD=hd.ID_HOADON and ChuyenNoKhoDoi=0 and NGAYGIAITRACH is not null)=(select count(*) from TT_CTDongNuoc where MaDN=dn.MaDN)");
        }
        ...
        public DataTable getDSKQDongNuoc_CanMoNuoc()
        {
            ...
            return ExecuteQuery_DataTable("select distinct MaKQDN,kqdn.MaDN,kqdn.CreateDate,kqdn.DanhBo,kqdn.HoTen,kqdn.DiaChi,NgayDN"
                        ...
                        + " CoDHN = Co,kqdn.GhiChuTroNgai,TroNgaiMN,MaNV_DongNuoc=(select HoTen from TT_NguoiDung where MaND=dn.MaNV_DongNuoc)"
                        + " from TT_DongNuoc dn,TT_KQDongNuoc kqdn,TT_CTDongNuoc ctdn"
                        + " where dn.MaDN=kqdn.MaDN and dn.MaDN=ctdn.MaDN and NgayDN is not null and DongPhi=1 and NgayMN is null and TroNgaiMN=0"
                        + " and (select count(*) from TT_CTDongNuoc a,HOADON hd where a.MaDN=dn.MaDN and a.MaHD=hd.ID_HOADON and ChuyenNoKhoDoi=0 and NGAYGIAITRACH is not null)=(select count(*) from TT_CTDongNuoc where MaDN=dn.MaDN)");
        }
        ...
        public DataTable getDSKQDongNuoc_TroNgaiMoNuoc()
        {
            ...
            return ExecuteQuery_DataTable("select distinct MaKQDN,kqdn.MaDN,kqdn.CreateDate,kqdn.DanhBo,kqdn.HoTen,kqdn.DiaChi,NgayDN"
                        ...
                        + " CoDHN = Co,kqdn.GhiChuTroNgai,TroNgaiMN,MaNV_DongNuoc=(select HoTen from TT_NguoiDung where MaND=dn.MaNV_DongNuoc)"
                        + " from TT_DongNuoc dn,TT_KQDongNuoc kqdn,TT_CTDongNuoc ctdn"
                        + " where dn.MaDN=kqdn.MaDN and dn.MaDN=ctdn.MaDN and NgayDN is not null and DongPhi=1 and NgayMN is null and TroNgaiMN=1"
                        + " and (select count(*) from TT_CTDongNuoc a,HOADON hd where a.MaDN=dn.MaDN and a.MaHD=hd.ID_HOADON and ChuyenNoKhoDoi=0 and NGAYGIAITRACH is not null)=(select count(*) from TT_CTDongNuoc where MaDN=dn.MaDN)");
        }
