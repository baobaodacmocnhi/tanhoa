    ...
    class CChotDangNgan : CDAL
    {
        ...
        public DataTable getDS(DateTime FromNgayChot, DateTime ToNgayChot)
        {
            string sql = "select ID,NgayChot,Chot,"
                        ...
                        + " SLGiay=(select COUNT(ID_HOADON) from HOADON where CAST(NGAYGIAITRACH as date)=CAST(NgayChot as date) and (NAM<2020 or (NAM=2020 and KY<6)) and MaNV_DangNgan is not null),"
                        + " TCGiay=(select SUM(TONGCONG) from HOADON where CAST(NGAYGIAITRACH as date)=CAST(NgayChot as date) and (NAM<2020 or (NAM=2020 and KY<6)) and MaNV_DangNgan is not null),"
                        + " SLHDDT=(select COUNT(ID_HOADON) from HOADON where CAST(NGAYGIAITRACH as date)=CAST(NgayChot as date) and (NAM>2020 or (NAM=2020 and KY>=6)) and MaNV_DangNgan is not null),"
                        + " TCHDDT=(select SUM(TONGCONG) from HOADON where CAST(NGAYGIAITRACH as date)=CAST(NgayChot as date) and (NAM>2020 or (NAM=2020 and KY>=6)) and MaNV_DangNgan is not null),"
                        ...
                        //+ " TCThanhToan=(select SUM(TONGCONG) from HOADON where CAST(NGAYGIAITRACH as date)=CAST(NgayChot as date) and SyncThanhToan=1),"
