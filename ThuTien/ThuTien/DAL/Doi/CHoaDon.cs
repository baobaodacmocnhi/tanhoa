...
namespace ThuTien.DAL.Doi
{
    class CHoaDon : CDAL
    {
        ...
        public DataTable GetTongHopDangNganChiTietTG(int MaTo, DateTime NgayGiaiTrach)
        {
            ///Tổ Văn Phòng
            if (_db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).HanhThu != true)
            {
            ...
            {
                var query = from itemND in _db.TT_NguoiDungs
                            join itemHD in _db.HOADONs on itemND.MaND equals itemHD.MaNV_HanhThu into tableHD
                            from itemtableHD in tableHD.DefaultIfEmpty()
                            where //Convert.ToInt32(itemtableHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                                //&& Convert.ToInt32(itemtableHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                                //&& _db.TT_NguoiDungs.Any(itemND2 => itemND2.MaTo == MaTo && itemND.MaND == itemtableHD.MaNV_DangNgan)///Kiểm tra nhân viên đăng ngân thuộc tổ
                                itemND.HanhThu==true && itemtableHD.NGAYGIAITRACH.Value.Date == NgayGiaiTrach.Date && itemtableHD.GB >= 11 && itemtableHD.GB <= 20
                            //orderby item.MaNV_DangNgan ascending
                            //group item by item.MaNV_DangNgan into itemGroup
                            select new
                            {
                                MaNV = itemND.MaND,
                                itemND.HoTen,
                                ChuyenKhoan = itemtableHD.DangNgan_ChuyenKhoan,
                                DanhBo = itemtableHD.DANHBA,
                                itemtableHD.GIABAN,
                                ThueGTGT = itemtableHD.THUE,
                                PhiBVMT = itemtableHD.PHI,
                                itemtableHD.TONGCONG,
                            } into x
                            group x by x.MaNV into g
                            select new
                            {
                                MaNV = g.Key,
                                g.FirstOrDefault().HoTen,
                                ChuyenKhoan = g.FirstOrDefault().ChuyenKhoan,
                                TongHD = g.Count(),
                                TongGiaBan = g.Sum(groupItem => groupItem.GIABAN),
                                TongThueGTGT = g.Sum(groupItem => groupItem.ThueGTGT),
                                TongPhiBVMT = g.Sum(groupItem => groupItem.PhiBVMT),
                                TongCong = g.Sum(groupItem => groupItem.TONGCONG),
                            };
                return LINQToDataTable(query);
            }
        }
        ...
        public DataTable GetDSTon(string Loai, int MaNV)
        {
            if (Loai == "TG")
            {
                var query = from item in _db.HOADONs
        ...
        public DataTable GetDSToTon(string Loai, int MaTo, int Nam, int Ky)
        {
            if (Loai == "TG")
            {
                var query = from itemHD in _db.HOADONs
                            ...
                            && itemHD.NAM == Nam && itemHD.KY == Ky && itemHD.NGAYGIAITRACH == null && itemHD.GB >= 11 && itemHD.GB <= 20
                ...
                if (Loai == "CQ")
                {
                    var query = from itemHD in _db.HOADONs
                                ...
                                where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                                && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS 
                                && itemHD.NAM == Nam && itemHD.KY == Ky && itemHD.NGAYGIAITRACH == null && itemHD.GB > 20
                                orderby itemHD.MALOTRINH ascending
                                select new
                                {
                                    itemHD.SOHOADON,
        ...
        public DataTable GetDSToTon(string Loai, int MaTo, int Nam)
        {
            if (Loai == "TG")
            {
                var query = from itemHD in _db.HOADONs
                            ...
                            where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                                && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                            && itemHD.NAM == Nam && itemHD.NGAYGIAITRACH == null && itemHD.GB >= 11 && itemHD.GB <= 20
                            orderby itemHD.MALOTRINH ascending
                            select new
                            {
                                itemHD.SOHOADON,
                ...
                if (Loai == "CQ")
                {
                    var query = from itemHD in _db.HOADONs
                                ...
                                where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                                && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                                && itemHD.NAM == Nam && itemHD.NGAYGIAITRACH == null && itemHD.GB > 20
                                orderby itemHD.MALOTRINH ascending
                                select new
                                {
                                    itemHD.SOHOADON,
        ...
        public DataTable GetDSToTon(string Loai, int MaTo)
        {
            if (Loai == "TG")
            {
                var query = from itemHD in _db.HOADONs
                ...
                if (Loai == "CQ")
                {
                    var query = from itemHD in _db.HOADONs
                                ...
                                where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                                && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                                && itemHD.NGAYGIAITRACH == null && itemHD.GB > 20
                                orderby itemHD.MALOTRINH ascending
                                select new
                                {
                                    itemHD.SOHOADON,
        ...
        public DataTable GetDSHoaDon0(int Nam, int Ky, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        join itemND in _db.TT_NguoiDungs on itemHD.MaNV_HanhThu equals itemND.MaND into tableND
                        from itemtableND in tableND.DefaultIfEmpty()
                        where itemHD.GIABAN == 0 && itemHD.NAM == Nam && itemHD.KY == Ky
        ...
        public DataTable GetDSHoaDon0(int Nam, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        join itemND in _db.TT_NguoiDungs on itemHD.MaNV_HanhThu equals itemND.MaND into tableND
                        from itemtableND in tableND.DefaultIfEmpty()
                        where itemHD.GIABAN == 0 && itemHD.NAM == Nam
                        && itemHD.GB.Value.ToString().Contains(GiaBieu.ToString())
                        && (itemHD.DM == null || itemHD.DM.Value.ToString().Contains(@DinhMuc.ToString()))
                        && itemHD.CODE.Contains(Code)
                        orderby itemHD.ID_HOADON descending
        ...
        public DataTable GetDSHoaDon0_To(int MaTo,int Nam, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        ...
                        where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                              && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                        && itemHD.GIABAN == 0 && itemHD.NAM == Nam
                        && itemHD.GB.Value.ToString().Contains(GiaBieu.ToString())
                        && (itemHD.DM == null || itemHD.DM.Value.ToString().Contains(@DinhMuc.ToString()))
                        && itemHD.CODE.Contains(Code)
                        orderby itemHD.ID_HOADON descending
        ...
        public DataTable GetDSHoaDon0_To(int MaTo,int Nam, int Ky, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        ...
                        where Convert.ToInt32(itemHD.MAY) >= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).TuCuonGCS
                              && Convert.ToInt32(itemHD.MAY) <= _db.TT_Tos.SingleOrDefault(itemTo => itemTo.MaTo == MaTo).DenCuonGCS
                        && itemHD.GIABAN == 0 && itemHD.NAM == Nam && itemHD.KY == Ky
                        && itemHD.GB.Value.ToString().Contains(GiaBieu.ToString())
                        && (itemHD.DM == null || itemHD.DM.Value.ToString().Contains(@DinhMuc.ToString()))
                        && itemHD.CODE.Contains(Code)
                        orderby itemHD.ID_HOADON descending
        ...
        public DataTable GetDSHoaDon0_NV(int MaNV, int Nam, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        join itemND in _db.TT_NguoiDungs on itemHD.MaNV_HanhThu equals itemND.MaND into tableND
                        from itemtableND in tableND.DefaultIfEmpty()
                        where itemHD.MaNV_HanhThu==MaNV && itemHD.GIABAN == 0 && itemHD.NAM == Nam
                        && itemHD.GB.Value.ToString().Contains(GiaBieu.ToString())
                        && (itemHD.DM == null || itemHD.DM.Value.ToString().Contains(@DinhMuc.ToString()))
                        && itemHD.CODE.Contains(Code)
                        orderby itemHD.ID_HOADON descending
        ...
        public DataTable GetDSHoaDon0_NV(int MaNV, int Nam, int Ky, string GiaBieu, string DinhMuc, string Code)
        {
            var query = from itemHD in _db.HOADONs
                        join itemND in _db.TT_NguoiDungs on itemHD.MaNV_HanhThu equals itemND.MaND into tableND
                        from itemtableND in tableND.DefaultIfEmpty()
                        where itemHD.MaNV_HanhThu == MaNV && itemHD.GIABAN == 0 && itemHD.NAM == Nam && itemHD.KY == Ky
                        && itemHD.GB.Value.ToString().Contains(GiaBieu.ToString())
                        && (itemHD.DM == null || itemHD.DM.Value.ToString().Contains(@DinhMuc.ToString()))
                        && itemHD.CODE.Contains(Code)
                        orderby itemHD.ID_HOADON descending
