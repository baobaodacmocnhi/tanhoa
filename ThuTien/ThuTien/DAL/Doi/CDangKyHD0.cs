    ...
    class CDangKyHD0 : CDAL
    {
        ...
        public DataTable GetDS(int MaNV, int Nam)
        {
            string sql = "declare @nam int;"
                        ...
                        + " set @MaNV="+MaNV+";"
                        + " select t1.* from"
                        + " (select distinct db.TenTo,db.MaNV,db.HoTen,db.DanhBo,db.DiaChi,ky1.Ky1,ky2.Ky2,ky3.Ky3,ky4.Ky4,"
                        + " ky5.Ky5,ky6.Ky6,ky7.Ky7,ky8.Ky8,ky9.Ky9,ky10.Ky10,ky11.Ky11,ky12.Ky12,row_number() over (partition by db.DanhBo order by db.DanhBo) as RowNumber"
                        + " from"
                        + " (select TenTo,MaNV,nd.HoTen,hd.DANHBA as DanhBo,hd.SO+' '+hd.DUONG as DiaChi"
