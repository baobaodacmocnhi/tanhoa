...
namespace ThuTien.DAL.Doi
{
    class CDangKy : CDAL
    {
        public bool Them(TT_DangKy dangky)
        {
            try
            {
                dangky.CreateDate = DateTime.Now;
                dangky.CreateBy = CNguoiDung.MaND;
                _db.TT_DangKies.InsertOnSubmit(dangky);
                _db.SubmitChanges();
                return true;
            }
            catch (Exception ex)
        ...
        public bool Xoa(TT_DangKy dangky)
        {
            try
            {
                _db.TT_DangKies.DeleteOnSubmit(dangky);
                _db.SubmitChanges();
                return true;
            }
            catch (Exception ex)
        ...
        public bool Sua(TT_DangKy dangky)
        {
            try
            {
        ...
        public bool CheckExist(string DanhBo,int MaNV)
        {
        ...
        public DataTable GetDS(int MaNV, int Nam)
        {
            string sql = "declare @nam int;"
                        ...
                        + " set @MaNV=" + MaNV + ";"
                        + " select t1.* from"
                        + " (select distinct db.TenTo,db.MaNV,db.HoTen,db.DanhBo,db.MLT,db.DiaChi,db.DM,ky1.Ky1,ky2.Ky2,ky3.Ky3,ky4.Ky4,"
                        ...
                        + " from"
                        + " (select TenTo,MaNV,nd.HoTen,hd.DANHBA as DanhBo,MLT=hd.MALOTRINH,hd.SO+' '+hd.DUONG as DiaChi,hd.ID_HOADON as MaHD,DM"
                        + " from TT_DangKy dk,HOADON hd,TT_NguoiDung nd,TT_To tto"
                        + " where dk.DanhBo=hd.DANHBA and dk.MaNV=nd.MaND and nd.MaTo=tto.MaTo and dk.MaNV=@MaNV) db"
                        + " left join"
                        + " (select dk.DanhBo,TIEUTHU as Ky1"
                        + " from TT_DangKy dk,HOADON hd"
