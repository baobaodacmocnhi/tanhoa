...
namespace ThuTien.DAL.QuanTri
{
    class CTo : CDAL
    {
        ...
        public List<TT_To> getDS(int IDPhong)
        {
            return _db.TT_Tos.Where(o => o.IDPhong == IDPhong).ToList();
        }
        ...
        public List<TT_To> getDS_HanhThu(int IDPhong)
        {
        ...
        {
            return _db.TT_Tos.Where(item => (item.HanhThu == true || item.DongNuoc == true) && item.An == false).ToList();
        }
