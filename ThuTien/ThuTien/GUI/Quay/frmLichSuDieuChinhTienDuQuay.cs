...
{
    public partial class frmLichSuDieuChinhTienDuQuay : Form
    {
        CTienDuQuay _cTienDuQuay = new CTienDuQuay();

        public frmLichSuDieuChinhTienDuQuay()
        {
            InitializeComponent();
        }
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            if (dateTu.Value <= dateDen.Value)
            {
                ...
                {
                    case "Bảng Kê":
                        dt = _cTienDuQuay.GetDSLichSu("Bảng Kê", dateTu.Value, dateDen.Value);
                        break;
                    case "Đăng Ngân":
                        dt = _cTienDuQuay.GetDSLichSu("Đăng Ngân", dateTu.Value, dateDen.Value);
                        break;
                    //case "Chuyển Tiền":
                    //    dt = _cTienDuQuay.GetDSLichSu("Chuyển Tiền", dateTu.Value, dateDen.Value);
                    //    break;
                    //case "Nhận Tiền":
                    //    dt = _cTienDuQuay.GetDSLichSu("Nhận Tiền", dateTu.Value, dateDen.Value);
                    //    break;
                    case "Điều Chỉnh Tiền":
                        dt = _cTienDuQuay.GetDSLichSu("Điều Chỉnh Tiền", dateTu.Value, dateDen.Value);
                        break;
                    default:
                        dt = _cTienDuQuay.GetDSLichSu("", dateTu.Value, dateDen.Value);
                        break;
                }
        ...
        private void txtDanhBo_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                dgvLichSuTienDu.DataSource = _cTienDuQuay.GetDSLichSu(txtDanhBo.Text.Trim().Replace(" ", ""));
                long TongCong = 0;
