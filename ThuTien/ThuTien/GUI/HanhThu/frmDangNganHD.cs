...
{
    public partial class frmDangNganHD : Form
    {
        CHoaDon _cHoaDon = new CHoaDon();

        public frmDangNganHD()
        {
            InitializeComponent();
        }
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            if (cmbKy.SelectedIndex != -1 && cmbDot.SelectedIndex != -1)
            {
                ...
                dgvHDDaThu.DataSource = _cHoaDon.GetDSDangNganByMaNVNamKyDot(CNguoiDung.MaND, int.Parse(cmbNam.SelectedValue.ToString()), int.Parse(cmbKy.SelectedItem.ToString()), int.Parse(cmbDot.SelectedItem.ToString()));
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            if (dgvTongHD.RowCount > 0 && flagThem && lstHD.Items.Count > 0)
            {
                ...
                foreach (var item in lstHD.Items)
                {
                    ...
                    {
                         MessageBox.Show("Danh Sách Hóa Đơn có sai", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                         return;
                    }
                }
            }
