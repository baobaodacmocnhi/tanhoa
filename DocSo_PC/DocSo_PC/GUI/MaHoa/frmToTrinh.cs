...
namespace DocSo_PC.GUI.MaHoa
{
    public partial class frmToTrinh : Form
    {
        ...
        {
            byte[] file = _wsDHN.get_Hinh_MaHoa("ToTrinh", _totrinh.ID.ToString(), dgvHinh.CurrentRow.Cells["Name_Hinh"].Value.ToString() + dgvHinh.CurrentRow.Cells["Loai_Hinh"].Value.ToString());
            if (file != null)
        ...
        private void btnXem_DS_Click(object sender, EventArgs e)
        {
            ...
            {
                case "Thời Gian":
                    dgvToTrinh.DataSource = _cToTrinh.getDS(CNguoiDung.FromDot, CNguoiDung.ToDot, dateTu_DS.Value, dateDen_DS.Value);
                    break;
                case "Mã Đơn":
